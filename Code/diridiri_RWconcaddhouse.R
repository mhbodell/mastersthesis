...
hist(rbind(out_conc[[1]],out_conc[[2]],out_conc[[3]]), breaks=50, main="", xlab="Concentration parameter", freq=FALSE, las=1)
...
text(x=mean(rbind(out_conc[[1]],out_conc[[2]],out_conc[[3]]))+5,y=max(density(rbind(out_conc[[1]],out_conc[[2]],out_conc[[3]]))$y) ,labels=round(mean(rbind(out_conc[[1]],out_conc[[2]],out_conc[[3]]))), cex=0.7)
...
for(i in 1:dim(rChain[[1]])[1]){
  for(j in 1:nrow(df)){
    ...
    alphas = (c(M,L,KD,C,S,MP,V,SD,O) + map_house[which(row.names(map_house)==h),])* df$n[j]
