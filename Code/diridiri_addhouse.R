...
#for(k in 2:npolls){
#for(i in 2:nparties){
#for(j in 2:(nhouses-1)){house[j,i,k] ~ dnorm(house[j,i,k-1], sigh)}
#}
#for (i in 2:nparties) { house[1, i,k] <- -sum(house[2:nhouses, i,k])}
#for(i in 1:(nhouses-1)) { house[i, 1,k] <- -sum(house[i, 2:nparties,k])}
#}
...
'
writeLines(diri_diri_lm,con="diri_diri_lm.bug")
system.time(jags_ddlm2010 <- jags.model("diri_diri_lm.bug", data = data, n.chain=3, n.adapt=50000))
ninter=10000
system.time(all_out22010 <- coda.samples(jags_ddlm2010,variable.names = c("x","conc","house", "sigd"), n.iter = ninter, thin = 5, burnin=1000))
