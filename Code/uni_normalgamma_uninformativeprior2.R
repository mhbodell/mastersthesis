...
system.time(jags_mod_M <- jags.model("kalman_M.bug", data = data_M, n.chains = 3, n.adapt = 100))

system.time(outM <- coda.samples(jags_mod_M,variable.names = c("xM", "M"), n.iter = ninter, n.thin = 1, n.chain=1))
...
system.time(jags_mod_L <- jags.model("kalman_L.bug", data = data_L, n.chains = 3, n.adapt = 100))

system.time(outL <- coda.samples(jags_mod_L,variable.names = c("xL", "L", "phiL"), n.iter = ninter, n.thin = 1))
...
system.time(jags_mod_KD <- jags.model("kalman_KD.bug", data = data_KD, n.chains = 3, n.adapt = 100))

system.time(outKD <- coda.samples(jags_mod_KD,variable.names = c("xKD", "KD", "phiKD"), n.iter = ninter, n.thin = 1))
...
system.time(jags_mod_C <- jags.model("kalman_C.bug", data = data_C, n.chains = 3, n.adapt = 100))

system.time(outC <- coda.samples(jags_mod_C,variable.names = c("xC", "C", "phiC"), n.iter = ninter, n.thin = 1))
...
system.time(jags_mod_S <- jags.model("kalman_S.bug", data = data_S, n.chains = 3, n.adapt = 100))
...
system.time(jags_mod_MP <- jags.model("kalman_MP.bug", data = data_MP, n.chains = 3, n.adapt = 100))
...
system.time(jags_mod_V <- jags.model("kalman_V.bug", data = data_V, n.chains = 3, n.adapt = 100))
...
system.time(jags_mod_SD <- jags.model("kalman_SD.bug", data = data_SD, n.chains = 3, n.adapt = 100))
...
set.seed(901207)
randchain = sample(1:ninter,1)
rsimM = outM[randchain,]
rsimL = outL[randchain,]
rsimKD = outKD[randchain,]
rsimC = outC[randchain,]
rsimS = outS[randchain,]
rsimMP = outMP[randchain,]
rsimV = outV[randchain,]
rsimSD = outSD[randchain,]
...
#rsims = cbind(as.numeric(rsimM[[1]]),as.numeric(rsimL[[1]]),as.numeric(rsimKD[[1]]),
#        as.numeric(rsimC[[1]]),as.numeric(rsimS[[1]]),as.numeric(rsimMP[[1]]),
#        as.numeric(rsimV[[1]]),as.numeric(rsimSD[[1]]))

#rsimsPolls = cbind(rsimM[[1]][which(regexpr("xM", row.names(sumM$statistics))==1)][datM$fieldDate.num],
#               rsimL[[1]][which(regexpr("xL", row.names(sumL$statistics))==1)][datL$fieldDate.num],
#               rsimKD[[1]][which(regexpr("xKD", row.names(sumKD$statistics))==1)][datKD$fieldDate.num],
#               rsimC[[1]][which(regexpr("xC", row.names(sumC$statistics))==1)][datC$fieldDate.num],
#               rsimS[[1]][which(regexpr("xS", row.names(sumS$statistics))==1)][datS$fieldDate.num],
#               rsimMP[[1]][which(regexpr("xMP", row.names(sumMP$statistics))==1)][datMP$fieldDate.num],
#               rsimV[[1]][which(regexpr("xV", row.names(sumV$statistics))==1)][datV$fieldDate.num],
#               rsimSD[[1]][which(regexpr("xSD", row.names(sumSD$statistics))==1)][datSD$fieldDate.num])
#row.names(rsimsPolls) = c(1:nrow(rsimsPolls))               

rep_perc90M = abs(apply(yrepM,1, function(s) s[floor(0.9*nrow(datM))]-rsimM[[1]][which(regexpr("xM", row.names(sumM$statistics))==1)][datM$fieldDate.num]))
rep_perc10M = abs(apply(yrepM,1, function(s) s[floor(0.1*nrow(datM))]-rsimM[[1]][which(regexpr("xM", row.names(sumM$statistics))==1)][datM$fieldDate.num]))
perc90M = abs(datM$M[floor(0.9*nrow(datM))] - rsimM[[1]][which(regexpr("xM", row.names(sumM$statistics))==1)][datM$fieldDate.num])
perc10M = abs(datM$M[floor(0.1*nrow(datM))] - rsimM[[1]][which(regexpr("xM", row.names(sumM$statistics))==1)][datM$fieldDate.num])
osM = sum(ifelse((rep_perc90M-rep_perc10M)>=(perc90M-perc10M),1,0)) / length(datM$M)

rep_perc90L = abs(apply(yrepL,1, function(s) s[floor(0.9*nrow(datL))]-rsimL[[1]][which(regexpr("xL", row.names(sumL$statistics))==1)][datL$fieldDate.num][floor(0.9*nrow(datL))]))
rep_perc10L = abs(apply(yrepL,1, function(s) s[floor(0.1*nrow(datL))]-rsimL[[1]][which(regexpr("xL", row.names(sumL$statistics))==1)][datL$fieldDate.num][floor(0.1*nrow(datL))]))
perc90L = abs(datL$L[floor(0.9*nrow(datL))] - rsimL[[1]][which(regexpr("xL", row.names(sumL$statistics))==1)][datL$fieldDate.num][floor(0.9*nrow(datL))])
perc10L = abs(datL$L[floor(0.1*nrow(datL))] - rsimL[[1]][which(regexpr("xL", row.names(sumL$statistics))==1)][datL$fieldDate.num][floor(0.1*nrow(datL))])
osL = sum(ifelse((rep_perc90L-rep_perc10L)>=(perc90L-perc10L),1,0)) / length(datL$L)

rep_perc90KD = abs(apply(yrepKD,1, function(s) s[floor(0.9*nrow(datKD))]-rsimKD[[1]][which(regexpr("xKD", row.names(sumKD$statistics))==1)][datKD$fieldDate.num][floor(0.9*nrow(datKD))]))
rep_perc10KD = abs(apply(yrepKD,1, function(s) s[floor(0.1*nrow(datKD))]-rsimKD[[1]][which(regexpr("xKD", row.names(sumKD$statistics))==1)][datKD$fieldDate.num][floor(0.1*nrow(datKD))]))
perc90KD = abs(datKD$KD[floor(0.9*nrow(datKD))] - rsimKD[[1]][which(regexpr("xKD", row.names(sumKD$statistics))==1)][datKD$fieldDate.num][floor(0.9*nrow(datKD))])
perc10KD = abs(datKD$KD[floor(0.1*nrow(datKD))] - rsimKD[[1]][which(regexpr("xKD", row.names(sumKD$statistics))==1)][datKD$fieldDate.num][floor(0.1*nrow(datKD))])
osKD = sum(ifelse((rep_perc90KD-rep_perc10KD)>=(perc90KD-perc10KD),1,0)) / length(datKD$KD)

rep_perc90C = abs(apply(yrepC,1, function(s) s[floor(0.9*nrow(datC))]-rsimC[[1]][which(regexpr("xC", row.names(sumC$statistics))==1)][datC$fieldDate.num][floor(0.9*nrow(datM))]))
rep_perc10C = abs(apply(yrepC,1, function(s) s[floor(0.1*nrow(datC))]-rsimC[[1]][which(regexpr("xC", row.names(sumC$statistics))==1)][datC$fieldDate.num][floor(0.1*nrow(datM))]))
perc90C = abs(datC$C[floor(0.9*nrow(datC))] - rsimC[[1]][which(regexpr("xC", row.names(sumC$statistics))==1)][datC$fieldDate.num][floor(0.9*nrow(datM))])
perc10C = abs(datC$C[floor(0.1*nrow(datC))] - rsimC[[1]][which(regexpr("xC", row.names(sumC$statistics))==1)][datC$fieldDate.num][floor(0.1*nrow(datM))])
osC = ifelse((rep_perc90C-rep_perc10C)>=(perc90C-perc10C),1,0)

rep_perc90S = abs(apply(yrepS,1, function(s) s[floor(0.9*nrow(datS))]-rsimS[[1]][which(regexpr("xS", row.names(sumS$statistics))==1)][datS$fieldDate.num][floor(0.9*nrow(datS))]))
rep_perc10S = abs(apply(yrepS,1, function(s) s[floor(0.1*nrow(datS))]-rsimS[[1]][which(regexpr("xS", row.names(sumS$statistics))==1)][datS$fieldDate.num][floor(0.1*nrow(datS))]))
perc90S = abs(datS$S[floor(0.9*nrow(datS))] - rsimS[[1]][which(regexpr("xS", row.names(sumS$statistics))==1)][datS$fieldDate.num][floor(0.9*nrow(datS))])
perc10S = abs(datS$S[floor(0.1*nrow(datS))] - rsimS[[1]][which(regexpr("xS", row.names(sumS$statistics))==1)][datS$fieldDate.num][floor(0.1*nrow(datS))])
osS = sum(ifelse((rep_perc90S-rep_perc10S)>=(perc90S-perc10S),1,0)) / length(datS$S)

rep_perc90MP = abs(apply(yrepMP,1, function(s) s[floor(0.9*nrow(datMP))]-rsimMP[[1]][which(regexpr("xMP", row.names(sumMP$statistics))==1)][datMP$fieldDate.num][floor(0.9*nrow(datMP))]))
rep_perc10MP = abs(apply(yrepMP,1, function(s) s[floor(0.1*nrow(datMP))]-rsimMP[[1]][which(regexpr("xMP", row.names(sumMP$statistics))==1)][datMP$fieldDate.num][floor(0.1*nrow(datMP))]))
perc90MP = abs(datMP$MP[floor(0.9*nrow(datMP))] - rsimMP[[1]][which(regexpr("xMP", row.names(sumMP$statistics))==1)][datMP$fieldDate.num][floor(0.9*nrow(datMP))])
perc10MP = abs(datMP$MP[floor(0.1*nrow(datMP))] - rsimMP[[1]][which(regexpr("xMP", row.names(sumMP$statistics))==1)][datMP$fieldDate.num][floor(0.1*nrow(datMP))])
osMP = sum(ifelse((rep_perc90MP-rep_perc10MP)>=(perc90MP-perc10MP),1,0)) / length(datMP$MP)

rep_perc90V = abs(apply(yrepV,1, function(s) s[floor(0.9*nrow(datV))]-rsimV[[1]][which(regexpr("xV", row.names(sumV$statistics))==1)][datV$fieldDate.num][floor(0.9*nrow(datV))]))
rep_perc10V = abs(apply(yrepV,1, function(s) s[floor(0.1*nrow(datV))]-rsimV[[1]][which(regexpr("xV", row.names(sumV$statistics))==1)][datV$fieldDate.num][floor(0.1*nrow(datV))]))
perc90V = abs(datV$V[floor(0.9*nrow(datV))] - rsimV[[1]][which(regexpr("xV", row.names(sumV$statistics))==1)][datV$fieldDate.num][floor(0.9*nrow(datV))])
perc10V = abs(datV$V[floor(0.1*nrow(datV))] - rsimV[[1]][which(regexpr("xV", row.names(sumV$statistics))==1)][datV$fieldDate.num][floor(0.1*nrow(datV))])
osV = sum(ifelse((rep_perc90V-rep_perc10V)>=(perc90V-perc10V),1,0)) / length(datV$V)

rep_perc90SD = abs(apply(yrepSD,1, function(s) s[floor(0.9*nrow(datSD))]-rsimSD[[1]][which(regexpr("xSD", row.names(sumSD$statistics))==1)][datSD$fieldDate.num][floor(0.9*nrow(datSD))]))
rep_perc10SD = abs(apply(yrepSD,1, function(s) s[floor(0.1*nrow(datSD))]-rsimSD[[1]][which(regexpr("xSD", row.names(sumSD$statistics))==1)][datSD$fieldDate.num][floor(0.1*nrow(datSD))]))
perc90SD = abs(datSD$SD[floor(0.9*nrow(datSD))] - rsimSD[[1]][which(regexpr("xSD", row.names(sumSD$statistics))==1)][datSD$fieldDate.num][floor(0.9*nrow(datSD))])
perc10SD = abs(datSD$SD[floor(0.1*nrow(datSD))] - rsimSD[[1]][which(regexpr("xSD", row.names(sumSD$statistics))==1)][datSD$fieldDate.num][floor(0.1*nrow(datSD))])
osSD = sum(ifelse((rep_perc90SD-rep_perc10SD)>=(perc90SD-perc10SD),1,0)) / length(datSD$SD)

######### sd #######

sdrepM = apply(yrepM,2,sd)
