...
for(l in 1:nparties){b[i,l] <- sum(z[1:k[i],l,i])/k[i]}
...
system.time(jags_ddlmRW <- jags.model("diri_diri_lm.bug", data = data, n.chain=3, n.adapt=100000))
ninter=200000
system.time(add_out <- coda.samples(jags_ddlmRW,variable.names = c("x","conc", "sigd"), n.iter = ninter, thin = 100, burnin=10000))
...
ind = sample(1:ncol(out_conc[[1]]),1)
plot(out_conc[[1]][,ind], main="", las=1)
plot(addout_x2[[1]][,ind], main="", las=1)
ind2 = sample(1:ncol(out_conc[[1]]),1)
plot(out_conc[[1]][,ind2], main="", las=1)
plot(addout_x2[[1]][,ind2], main="", las=1)
...
data = list(nperiod = df$Day[nrow(df)]+as.numeric(julian(elec$Date[2], df$endDate[nrow(df)])),k=df$length,
            ...
            z=array(NA, dim=c(df$Day[nrow(df)]+df$length[nrow(df)]+as.numeric(julian(elec$Date[2], df$endDate[nrow(df)])),9,nrow(df))),
...
ninter=200000
system.time(all_outRW22010 <- coda.samples(jags_ddlmRW2010,variable.names = c("x","conc", "sigd"), n.iter = ninter, thin = 100, burnin=10000))
...
df$house = factor(df$house, levels=c("Demoskop","Inizio", "Ipsos" ,"Novus" ,"SCB" ,"Sentio" ,
                                     "Sifo" ,"Skop" ,"SVT", "United Minds", "YouGov","Election"))
df = df[-which(df$endDate>=end.date),]
data = list(nperiod = df$Day[nrow(df)]+as.numeric(julian(elec$Date[3], df$endDate[nrow(df)]))+1,k=df$length,
            npolls = nrow(df),  nparties = 9, y = df[,partynames], day = df$Day,
            n = df$n, b=matrix(NA,ncol=9, nrow=nrow(df)), 
            z=array(NA, dim=c(df$Day[nrow(df)]+as.numeric(julian(elec$Date[3], df$endDate[nrow(df)]))+1,9,nrow(df))))
...
ninter=200000
system.time(all_out22014 <- coda.samples(jags_ddlm,variable.names = c("x","conc","sigd"), n.iter = ninter, thin = 100, burnin=10000))
sum_all22014 = summary(all_out22014)
add_out2014 = all_out22014[,which(regexpr("x", row.names(sum_all22014$statistics))==1)]


nperiods=df$Day[nrow(df)]+as.numeric(julian(elec$Date[3], df$endDate[nrow(df)])) + 1
...
data = list(nperiod = df$Day[nrow(df)]+as.numeric(julian(elec$Date[2], df$endDate[nrow(df)])),k=df$length,
            npolls = nrow(df),  nparties = 9, y = df[,partynames], day = df$Day,
            n = df$n, b=matrix(NA,ncol=9, nrow=nrow(df)), org = as.numeric(df$house),
            z=array(NA, dim=c(df$Day[nrow(df)]+as.numeric(julian(elec$Date[2], df$endDate[nrow(df)])),9,nrow(df))),
            nhouses = length(levels(as.factor(df$house))))
